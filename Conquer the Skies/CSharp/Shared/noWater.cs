            ...
            else
            {
                ...
                if (__instance.currentHull.WaterPercentage > 0.001f)
                {
                    ...
                    if (__instance.Collider.SimPosition.Y < waterSurfaceY)
                    {
                        ...
                        {
							if (waterSurfaceY < Double.PositiveInfinity) { //gdamn if yor ass is in gap between hull and outside its count water surface like infinity. i am not gonna comprehend math above
								__instance.inWater = true;
							}
                        }
                    }
