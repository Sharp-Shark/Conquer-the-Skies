...
if File.Exists(CTS.pathOverhaul .. '/Lua/CTS/secret.lua') then
	require 'CTS/secret'
end
...
local function discordChatMessage (message, hook)
	if CLIENT or (not Game.ServerSettings.IsPublic) or (Game.ServerSettings.HasPassword) then return end
...
Hook.Add("client.connected", "CTS.discordClientConnect", function (connectedClient)
	if SERVER and Game.ServerSettings.IsPublic and (not Game.ServerSettings.HasPassword) then
